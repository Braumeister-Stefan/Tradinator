...
class PerformanceMonitoring:
    ...
    def _build_pie_chart_data(self, analytics: dict) -> list[dict]:
        ...
        conId so each underlying market appears only once.
        """
        positions = analytics.get("positions") or []

        # Aggregate notional value by conId across all position tickets.
        ...
        for pos in positions:
            ...
            value = size * level
            if value > 0:
                raw_id = pos.get("conId") or "Unknown"
        ...
        for raw_id, value in aggregated.items():
            ...
            # back to the full conId for canonical symbols (e.g. "DAX").
            parts = raw_id.split(".")
